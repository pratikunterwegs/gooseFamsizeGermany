...
I also tried an alternative approach, using log-log transformed data on total families and juvnenile counts to run a simple linear model. The model fits the data and appears to explain much of the variation (adjusted R-squared = ```r summary(fam_juvs_lm1)$adj.r.squared```).
...
ggplot(data = fams, aes(x = juvs/fams, y = Mean_Fam))+
  geom_point(size = 0.5, pch = 21)+
...
The mean global family size is ```r mean(fams$Mean_Fam, na.rm = T)```. 
