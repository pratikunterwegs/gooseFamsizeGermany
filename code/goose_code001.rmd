...
Some values are inherently strange, such as the maximum percentage of juveniles (124%) in a flock. Other discrepancies, such as the mismatch between the maximum flock size and the maximum number of Whitefronts seen is probably due to missing values in the data. Both of these are easily fixed by using two of the five variables to calculate the missing ones.
...
1. Proportion of geese in a flock that are in a family. This should help determine how much of the flock receives the same level of benefit in inter-goose aggression.
