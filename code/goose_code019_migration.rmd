...
```{r mean_maxs}
#'find the first or last date, for autumn and spring respectively, for greater than 95th percentile values of activity in each year and each season

peaks = a1 %>% group_by(Year, season) %>% filter(percentile > 95) %>% summarize(peak = if(season == "spring") last(Date) else first(Date))

#'add peaks to df
a1 = merge(a1, peaks, by = c("Year","season"))

#'get time to peak from each record
a1$timetopeak = as.numeric(abs(difftime(a1$Date, a1$peak, units = "days")))
```
