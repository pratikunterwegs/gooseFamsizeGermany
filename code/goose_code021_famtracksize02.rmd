...
```{r}
library(ggplot2)
#load("tracked_family_sizes.RData")
...
#'get attrs at split
splits = lapply(splits, function(x){rbind(x[1,],x[which(diff(x$fsize)<0)+1,], x[dim(x)[1],])})
...
ggplot()+
  geom_path(data=splits, aes(x=lon, y = lat, col = fam), size = 2)+
  geom_
```
