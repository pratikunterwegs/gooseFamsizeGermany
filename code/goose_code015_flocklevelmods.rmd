...
flockmod = glmer(flocksize ~ distkol + mintemp + windspeed + t_since_in + p.index + (1|Breeding_year) + (1|Observer) + (1|Food_type), data = geese, family = poisson)
...
famsmod = gam(fams ~ s(flocksize) + mintemp + windspeed + distkol + p.index+ s(Breeding_year, bs = "re") + s(t_since_in, k = 4) +s(Observer, bs = "re") + s(Food_type, bs = "re"), data = fams2, family = poisson)
```
...
modjuvs = gam(propjuv ~ distkol + flocksize + mintemp + windspeed + p.index + s(t_since_in) + s(Breeding_year, bs = "re") + s(Observer, bs = "re") + s(Food_type, bs = "re"), family = binomial, data = geese)
