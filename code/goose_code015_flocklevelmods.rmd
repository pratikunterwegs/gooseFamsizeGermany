...
```{r glmer_fams}
famsmod = bam(fams ~ flocksize + p.index + lon + s(Breeding_year, bs = "re") + s(Observer, bs = "re") + s(Food_type, bs = "re"), data = fams[fams$flocksize > 0,], family = poisson)
```
...
save(gamjuvs, flockmod, famsmod, mod03, mod03.gorg, file = "goosemods_pres.RData")
```
