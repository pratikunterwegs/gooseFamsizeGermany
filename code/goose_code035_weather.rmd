...
nfams.tempmod = bam(fams ~ flocksize + s(t_since_in, k = 4) + ti(mintemp) + ti(ppt) + ti(mintemp, t_since_in) + s(Breeding_year, bs = "re") + s(Observer, bs = "re") + s(Food_type, bs = "re"), data = fams, family = "poisson")
...
```{r flocksize_temp}
flock.tempmod = bam(flocksize ~ distkol + ti(mintemp) + ti(ppt) + ti(mintemp, t_since_in) + s(Observer, bs = "fs") + s(Food_type, bs = "fs"), data = geese, family = "poisson")
...
```{r juvsclim}
juvs.tempmod = bam(propjuv ~ s(flocksize) + s(t_since_in, k = 4) + ti(mintemp) + ti(ppt) + s(Breeding_year, bs = "re"), data = geese, family = "binomial")

summary(juvs.tempmod)
```
