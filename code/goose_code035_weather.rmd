...
```{r load_sites}
library(readr)
knmi_sites <- read_csv("~/git/thesis/code/knmi_sites.txt")
```
...
```{r}
deweather <- read_csv("~/git/thesis/code/weather_munster.csv", 
   col_types = cols(date = col_date(format = "%Y%m%d")))
```
...
nfams.tempmod = bam(fams ~ s(flocksize) + distkol + s(t_since_in, k = 4) + mintemp + ppt + windspeed + s(Breeding_year, bs = "re") + s(Observer, bs = "re") + s(Food_type, bs = "re"), data = fams2, family = "poisson")

nfamspred = predict(nfams.tempmod, type = "response", terms = "s(flocksize)", newdata = fams2, se.fit = T)

summary(nfams.tempmod)

pdf(file = "~/git/thesis/texts/nfamsclim.pdf", width = 5, height = 5, title = "nfamsclim")
...
```{r flocksize_temp}
flock.tempmod = bam(flocksize ~ distkol + mintemp + ppt + windspeed + t_since_in + s(Observer, bs = "fs") + s(Food_type, bs = "fs"), data = geese, family = "poisson")

summary(flock.tempmod)

pdf(file = "~/git/thesis/texts/flockclim.pdf", width = 5, height = 5, title = "flockclim")
