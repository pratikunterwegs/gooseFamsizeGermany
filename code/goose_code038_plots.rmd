...
fig6d = 
  ggplot()+
  geom_smooth(data = fig5cdata %>% filter(variable == "pjuv"), aes(x = t_since_in, y = mean, ymin = mean -ci, ymax = mean+ci, group = variable), fill = "grey", lwd = 0.5, col = 1, method = "glm")+
  #geom_line(aes(x = flocksize, y = mean, ymin = mean -ci, ymax = mean+ci, group = variable), lwd = 0.5)+
...
fig6c = 
  ...
  geom_smooth(aes(x = flocksize, y = mean, ymin = mean -ci, ymax = mean+ci, group = variable), fill = "grey", lwd = 0.5, col = 1, method = "loess")+
  #geom_line(aes(x = flocksize, y = mean, ymin = mean -ci, ymax = mean+ci, group = variable), lwd = 0.5)+
  
  geom_pointrange(aes(x = flocksize, y = mean, ymin = mean-ci, ymax = mean+ci), shape = 21, fill = "white")+
  
  #facet_wrap(~ifelse(variable == "pfam.pred", "(b)", "(c)"), scales = "free")+
  coord_cartesian(xlim = c(0,2100), ylim = c(0,0.6))+
...
```{r}
#'export to eps
cairo_pdf(filename = "figs/fig6.eps", height = 5, width = 8)
