...
ggplot()+
  ...
  geom_pointrange(aes(x = famsize, y = mean, ymin = mean-ci, ymax = mean+ci, shape = stage, fill = stage), position = position_dodge(width = 0.2), fatten = 10, lwd = 0.2)+
  
  facet_wrap(~ifelse(data == "fams", "(a)", "(b)"), scales = "fixed")+
...
fig3data = fams.expand %>% 
  ...
  filter(t_since_in <= 200) %>% 
  group_by(data, days = cut(t_since_in, breaks = seq(0,180,10))) %>% 
...
ggplot()+
  ...
  geom_pointrange(data = fig3data, aes(x = days, y = mean, ymin = mean - ci, ymax = mean + ci, shape = data, fill = data), fatten = 10, stroke = 0.6, lwd = 0.2, position = position_dodge(width = 5))+
  
  geom_smooth(data = fig5data1 %>% filter(variable == "pfamjuvs.pred"),
  aes(x = days, y = (1 - mean)*4), col = 1, lty = 4, method = "glm", lwd = 0.5)+
  
  geom_pointrange(data = fig5data1 %>% filter(variable == "pfamjuvs.pred"), aes(x = days, y = (1 - mean)*4, ymin = (1 - mean-ci)*4, ymax = (1-mean+ci)*4), stroke = 0.6, lwd = 0.2, fatten = 10, shape = 24, fill = "white")+
...
ggplot(data = splits %>% filter(fam %in% c("Ev","Wo","Ha","Na","Ti","Ma","Ro","An","Jo")))+
  geom_line(aes(x = t_since_in, y= juvs, group = fam), lty = 1)+
  geom_point(aes(x = t_since_in, y = juvs, shape = event), fill = "grey", size = 2)+
  
  scale_shape_manual(values = c(16, 24, 21))+
  ...
  theme(legend.position = "none", legend.direction = "vertical",legend.key = element_blank(), strip.background = element_rect(fill = "grey90", colour = "transparent"), strip.text = element_text(face = "bold"))
...
```{r export fig3}
#'export to ps
cairo_ps(filename = "figs/fig4.eps", height = 5, width = 10)
