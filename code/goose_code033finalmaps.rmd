...
tm_shape(land, bbox = e - c(0,0,-0.2,0))+
  tm_polygons(col = "darkseagreen1", border.col = "grey10", alpha = 0.7)+
...
tm_shape(splits2[splits2$event == "Split",])+
  tm_symbols(shape = "event", col = "indianred1", shapes = c(25, 4), size = 0.5, border.col = "grey10", showNA = F, shapeNA = NA, legend.shape.show = F)+

tm_format_NLD()+
tm_scale_bar(position = c("left","bottom"), breaks=c(0,25,50), size = 1)+
  tm_compass(type = "8star", position = c("LEFT","TOP"), size = 4)+

tm_layout(frame = TRUE, bg.color = "slategray1")
dev.off()
```
