...
pos = coordinates(geeseorg.spatial)
...
```{r map_sovon}
#postscript(file = "~/git/thesis/texts/datamap.eps", horizontal = FALSE, onefile = FALSE)
#'map locations
map01 = tm_shape(land, bbox = e - c(0,0,-0.2,0))+
  tm_polygons(col = "white", border.col = "grey70", alpha = 0.7)+
#'country boundaries?
  #tm_shape(admin)+ tm_lines(col = "grey80")+
tm_shape(kdegeese2)+
...
tm_shape(fams.spatial)+
  tm_dots(col = "black", shape = 4, size = 0.5)+

#'family event paths?
#tm_shape(famlines)+ tm_lines(col = "black", lty = "dotted")+
  
tm_shape(splits2)+
  tm_symbols(shape = "event", shapes = c(25, 4), size = 0.5, col = "grey50", showNA = F, shapeNA = NA, border.col = "grey20", legend.shape.show = F)+
