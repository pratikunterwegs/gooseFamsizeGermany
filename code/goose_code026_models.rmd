---
title: Hypothesis testing 1 - 5, family level data
---
```{r set_env}
#'load knitr
...
Data used: Family size data (51,000 families), and geese.org data (12,500 individual observations with pair duplicates removed).
...
mod01 = bam(famsize ~ s(lon) + flocksize + days, data = fams.expand, family = "poisson")
```
...
mod02 = bam(famsize ~ s(lon) + flocksize + t_since_in + t_to_out + s(Food_type, bs = "re") + s(Observer, bs = "re") + s(Breeding_year, bs = "re"), data = fams.expand, family = "poisson")
```
...
mod03 = bam(famsize ~ s(lon) + flocksize + t_since_in + s(Food_type, bs = "re") + s(Observer, bs = "re") + s(Breeding_year, bs = "re"), data = fams.expand, family = "poisson")
```
...
```{r vis.gam03}
vis.gam(mod03, view = c("lon", "t_since_in"), type = "response", color = "terrain", theta = -45)
...
```{r gamm}
mod06 = gamm(famsize ~ s(lon) + s(lat) + ti(lon,lat) + flocksize + t_since_in, random = list(Breeding_year=~1, Food_type=~1, Observer=~1), data = fams.expand, family = poisson(), na.action = na.exclude)
```
...
Save gams and check for autocorrelation in another script.
