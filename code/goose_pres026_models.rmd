...
```{r}
pdf(file = "/home/pratik/git/thesis/texts/fig03b.pdf", height = 4, width = 4)

ggplot()+
  geom_path(data = splits %>% filter(event %in% c("Start", "Split")), aes(x = t_since_in, y = fsize, group = fam), arrow = arrow(length = unit(0.02, "npc"), type = "closed"), lty = 1, lwd = 0.2, col = "black")+
  
 geom_text(data = splits %>% filter(fam %in% c("Ev","Wo","Ha","Na","Ti","Ma","Ro","An","Jo"), event == "Start"), aes(x=t_since_in, y = fsize, label = substr(fam,1,1), angle = 0), hjust = 1, vjust = 0, size = 4)+
  labs(list(x = "Days since arrival", y = "Family size"))+ 
  ylim(1,7.2)+ xlim(30,180)+
  theme_bw()+a
