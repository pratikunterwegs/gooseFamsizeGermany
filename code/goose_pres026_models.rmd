...
p1 = ggplot()+
  ...
  labs(list(x = NULL, y = "Dist. (km) breeding grounds", title = "a < 60 days post arrival"))+
...
#  geom_segment(data = splits %>% filter(event %in% c("Split")), aes(x = lon, y = fsize+1, xend = lon, yend = 0), lty = 1, lwd = 0.2, col = "grey30")+
 ...
 geom_text(aes(x =8, y = 3e3, label = "{chi}^2==1.135"), parse = T, hjust = "inwards")+
  theme_bw()+a
#dev.off()
```
...
p2 = ggplot()+
  ...
  labs(list(x = NULL, y = NULL, title = "b > 60 days post arrival"))+
  geom_jitter(data = fams.expand.sub %>% filter(t_since_in > 60), aes(x = famsize, y = distkol), size = 0.1)+

 geom_text(aes(x =8, y = 3e3, label = "{chi}^2==4.194"), parse = T, hjust = "inwards")+
  theme_bw()+b
#dev.off()
```
...
p3 = ggplot()+
 ...
 geom_text(aes(x =8, y = 3.1e3, label = "{chi}^2==2.179"), parse = T, hjust = "inwards")+
  theme_bw()+a
#dev.off()
```
...
p4 = ggplot()+
 ...
 geom_text(aes(x =8, y = 3.1e3, label = "{chi}^2==0.023"), parse = T, hjust = "inwards")+ xlim(0,10)+
  theme_bw()+b
#dev.off()
```
...
p8 = ggplot()+
  ...
  geom_text(aes(x = 1.8, y = c(2.3, 1.5,0.75), label = c("1","2","3")))+
...
p12 = ggplot()+
  ...
  geom_text(aes(x = 1.8, y = 0.8, label = "{chi}^2==0.021"), hjust = "inward", parse = T)+ylim(0,0.8)+
  theme_bw()+a
