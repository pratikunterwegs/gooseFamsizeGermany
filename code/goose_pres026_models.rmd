...
```{r pred_gamjuvs_time}
vis.juvs.time = visreg(modjuvs, "t_since_in",plot = F)
...
ggplot()+
  ...
  geom_point(data = geese %>% dplyr::select(month, t_since_in, propjuv) %>% group_by(t_since_in) %>% dmap(mean), aes(x = t_since_in, y = propjuv), size = 1, shape = 1)+
