...
ggplot()+
  geom_point(data = geese %>% slice_rows("lookup") %>% dmap(mean), aes(x = distkol, y = flocksize), size = 3, pch = 1)+
...
```{r pred_gamjuvs_time}
vis.juvs.time = visreg(modjuvs, "t_since_in", plot = T, scale = "response")
...
ggplot()+
  geom_point(data = geese %>% dplyr::select(month, t_since_in, propjuv) %>% group_by(t_since_in) %>% dmap(mean), aes(x = t_since_in, y = propjuv), size = 2, shape = 1)+
  geom_ribbon(aes(x = geese$t_since_in, ymin = x$fit-1.96*x$se.fit, ymax = x$fit+1.96*x$se.fit), fill = "grey90", alpha = 0.5)+
  
  #geom_hex(data = geese, aes(x = t_since_in, y = propjuv))+
  #scale_fill_gradientn(colours=jet.col(20))+
  
  geom_line(aes(x = geese$t_since_in, y = x$fit))+
...
visfamsmod2 = visreg(famsmod, "flocksize", scale = "response", plot = F)

visfamsmod.data2 = visfamsmod2$fit
```

```{r plot_mod}
png(file = "~/git/thesis/texts/nfams_flock.png", width = 800, height = 800, res = 300, title = "nfams_dist")

ggplot()+
 geom_ribbon(data = visfamsmod.data2, aes(x = flocksize, ymin = visregLwr, ymax = visregUpr), fill = "grey90")+
  geom_point(data = fams2 %>% group_by(flocksize) %>% dmap(mean), aes(x = flocksize, y = fams), shape = 1, size = 2)+
  geom_line(data = visfamsmod.data2, aes(x = flocksize, y = visregFit), size = 1, col = 1)+
  labs(list(x = "Flock size", y = "Number of successful families"))+
  theme_bw()+a
