...
png(file = "~/git/thesis/texts/dist_fam.png", height = 800, width = 800, res = 300, title = "distance_famsize")
...
geom_point(data = fams.expand.sub %>% filter(t_since_in > 60) %>% slice_rows("famsize") %>% dmap(mean), aes(x = famsize, y = distkol), col = "royalblue", size = 3, pch = 16)+
...
geom_point(data = geeseorg %>% filter(t_since_in > 60) %>% slice_rows("famsize") %>% dmap(mean), aes(x = famsize, y = distkol), col = "royalblue", size = 3, pch = 17)+
...
png(file = "~/git/thesis/texts/fam_time.png", height = 800, width = 800, res = 300, title = "famsize_time")
...
ggplot()+
 ...
 geom_text(data = splits %>% filter(fam %in% c("Ev","Wo","Ha","Na","Ti","Ma","Ro","An","Jo"), event == "Start"), aes(x=t_since_in, y = fsize, label = substr(fam,1,1), angle = 0), hjust = 1, vjust = 0, size = 3)+
  ...
  labs(list(x = "Days since arrival", y = "N juveniles"))+ 
  ylim(0,8)+ #xlim(0,200)+
  theme_bw()+a
...
```{r plot_famsize_pred}
png(file = "~/git/thesis/texts/fam_predation.png", height = 800, width = 800, res = 300, title = "distance_famsize")

ggplot()+
  geom_point(data = geeseorg %>% slice_rows("p.index") %>% dmap(mean), aes(x = p.index, y = famsize), size = 3, pch = 17, col = "royalblue")+
 
  geom_point(data = geeseorg %>% filter(famsize > 0) %>% slice_rows("p.index") %>% dmap(mean), aes(x = p.index, y = famsize), size = 3, pch = 17, col = "grey10")+
   
  geom_point(data = fams.expand.sub %>% slice_rows("p.index") %>% dmap(mean), aes(x = p.index, y = famsize), size = 3,col="indianred1")+
...
labs(list(x = "Predation index", y = "Mean n juveniles"))+
  theme_bw()+a
...
png(file = "~/git/thesis/texts/flock_dist.png", width = 800, height = 800, res = 300, title = "flock_dist")
ggplot()+
...
```{r}
png(file = "~/git/thesis/texts/juvprop_time.png", width = 800, height =800, res = 300)

ggplot()+
  geom_ribbon(data = vis.juvs.time.data, aes(x = t_since_in, ymin = visregLwr, ymax = visregUpr), fill = "grey90", alpha = 0.9)+
  geom_point(data = geese %>% group_by(t_since_in) %>% dmap(mean), aes(x = t_since_in, y = propjuv), size = 1, shape = 1)+
...
```{r}
png(file = "~/git/thesis/texts/nfams_time.png", width = 800, height = 800, res = 300, title = "nfams_time")

ggplot()+
  geom_ribbon(data = vis.nfams.time.data, aes(x = t_since_in, ymin = visregLwr, ymax = visregUpr), fill= "grey90")+
  geom_point(data = fams %>% group_by(t_since_in) %>% dmap(mean), aes(x = t_since_in, y = fams), size = 1, shape = 1)+
...
```{r plot_pred_famsmod}
png(file = "~/git/thesis/texts/nfams_dist.png", width = 800, height = 800, res = 300, title = "nfams_dist")

ggplot()+
 geom_ribbon(data = visfamsmod.data, aes(x = distkol, ymin = visregLwr, ymax = visregUpr), fill = "grey90")+
  geom_point(data = fams2 %>% group_by(distkol) %>% dmap(mean), aes(x = distkol, y = fams), shape = 1, size = 3)+
