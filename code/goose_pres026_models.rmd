...
ggplot()+
  geom_point(data = geese %>% dplyr::select(month, t_since_in, propjuv) %>%  group_by(t_since_in) %>% dmap(mean), aes(x = t_since_in, y = propjuv), size = 1, shape = 1, col = "grey50")+

  geom_ribbon(aes(x = geese$t_since_in, ymin = x$fit-1.96*x$se.fit, ymax = x$fit+1.96*x$se.fit), fill = "grey90", alpha = 0.4)+
  
  geom_line(aes(x = geese$t_since_in, y = x$fit), lwd = 1)+
...
ggplot()+
  ...
  geom_point(data = fams %>% group_by(t_since_in) %>% dmap(mean), aes(x = t_since_in, y = fams), size = 3, shape = 1, col = "grey50")+
...
```{r plot_pred_famsmod}
pdf(file = "~/git/thesis/texts/nfams_dist.pdf", width = 4, height = 3, title = "nfams_dist")
...
```{r plot_mod}
pdf(file = "~/git/thesis/texts/nfams_flock.pdf", width = 4, height = 3, title = "nfams_dist")

ggplot()+
 geom_ribbon(data = visfamsmod.data2, aes(x = flocksize, ymin = visregLwr, ymax = visregUpr), fill = "grey90")+
  geom_point(data = fams2 %>% group_by(flocksize) %>% dmap(mean), aes(x = flocksize, y = fams), shape = 1, size = 2, colour = "grey50")+
...
ggplot()+
  geom_point(data=geese %>% mutate(roundflock = mround(flocksize, 25)) %>% slice_rows("roundflock") %>% dmap(mean), aes(x = flocksize, y = propjuv), size = 2, col = "grey50", shape = 1)+
 geom_ribbon(data = vis.juvs.flock.data, aes(x = flocksize, ymin = visregLwr, ymax = visregUpr), fill = "grey90", alpha = 0.5)+
  geom_line(data = vis.juvs.flock.data, aes(x = flocksize, y = visregFit), size = 1, col = 1)+
