...
p1 = ggplot()+
  ...
  labs(list(x = "Family size", y = "Dist. (km) breeding grounds", title = "a"))+
...
#  geom_segment(data = splits %>% filter(event %in% c("Split")), aes(x = lon, y = fsize+1, xend = lon, yend = 0), lty = 1, lwd = 0.2, col = "grey30")+
 ...
 geom_text(aes(x =8, y = 3e3, label = "{chi}^2==0.2866"), parse = T, hjust = "inwards")+
  theme_bw()+a
#dev.off()
```
...
p2 = ggplot()+
  ...
  labs(list(x = "Family size", y = "Dist. to breeding grounds (km)", title = "b"))+
  geom_jitter(data = fams.expand.sub %>% filter(t_since_in > 60), aes(x = famsize, y = distkol), size = 0.1)+

 geom_text(aes(x =8, y = 3e3, label = "{chi}^2==0.0406"), parse = T, hjust = "inwards")+
  theme_bw()+b
#dev.off()
```
...
p3 = ggplot()+
  ...
  labs(list(x = "Family size", y = "Dist. to breeding grounds (km)", title = "c"))+
  geom_jitter(data = geeseorg %>% filter(t_since_in < 60), aes(x = famsize, y = distkol), size = 0.1)+

 geom_text(aes(x =8, y = 3.1e3, label = "{chi}^2==0.1399"), parse = T, hjust = "inwards")+
  theme_bw()+a
#dev.off()
```
...
p4 = ggplot()+
  ...
  labs(list(x = "Family size", y = "Dist. to breeding grounds (km)", title = "d"))+
  geom_jitter(data = geeseorg %>% filter(t_since_in > 60), aes(x = famsize, y = distkol), size = 0.1)+

 geom_text(aes(x =8, y = 3.1e3, label = "{chi}^2==0.8785"), parse = T, hjust = "inwards")+ xlim(0,10)+
  theme_bw()+b
#dev.off()
```
...
p5 = ggplot()+
  ...
  geom_text(data = splits %>% filter(fam %in% c("Ev","Wo","Ha","Na","Ti","Ma","Ro","An","Jo"), event == "Start"), aes(x=t_since_in, y = fsize, label = substr(fam,1,1), angle = 0), hjust = 1, vjust = 0, size = 3)+
   ...
   geom_text(aes(x =180, y = 7, label = "{chi}^2==76.94"), parse = T, hjust = "inwards")+
...
p7 = ggplot()+
  ...
  ylim(0,5000)+
  labs(list(x = "Dist. to breeding grounds (km)", y = "Flock size", title = "a"))+
...
#p8 = ggplot()+
  ...
  labs(list(x = "Flock size", y = "Family size", title = "d"))+
  geom_text(data = NULL, aes(x = 1e4, y = 9.5, label = "{Omega[0]}^2==0.562"), hjust = "inward", parse = T)+
  theme_bw()+a
...
p11 = ggplot()+
  ...
  ylim(0.05, 0.7)+
  geom_text(aes(x = 200, y = 0.7, label = "{chi}^2==21.31"), hjust = "inward", parse = T)+
  theme_bw()+a
...
p12 = ggplot()+
 ...
 geom_ribbon(data = vis.juvs.flock.data, aes(x = flocksize, ymin = visregLwr, ymax = visregUpr), fill = "grey90", alpha = 0.9)+
  ...
  geom_text(aes(x = 9000, y = 0.7, label = "{chi}^2==5.952"), hjust = "inward", parse = T)+
  theme_bw()+b
...
p12 = ggplot()+
  ...
  geom_text(aes(x = 1.5, y = 0.8, label = "R^2==0.104"), hjust = "inward", parse = T)+
  theme_bw()+a
...
p14 =  ggplot()+
  ...
  geom_text(aes(x = 1.45, y = c(7.5,6.5,5.5), label = c("{Omega[0]}^2~1==0.661", "{Omega[0]}^2~2==0.563","{Omega[0]}^2~3==0.786")), hjust = "inward", parse = T)+
...
# Export plots
```{r}
save(p1,p2,p3,p4,p5,p6,p7,p8,p9, p11, p12, file = "goose_dataplots.Rdata")
```
