...
fes.early = fams.expand.sub %>% filter(t_since_in < 60) %>% mutate(pred.distmod = predict(distmod, newdata = ., type = "response", allow.new.levels = T, re.form = ~(1|Breeding_year)+(1|Observer)+(1|Food_type))) %>% group_by(famsize) %>% summarise(d.mean = mean(distkol, na.rm = T), d.sd = sd(distkol, na.rm=T), d.n = length(distkol), pred.mean = mean(pred.distmod, na.rm = T), pred.sd = sd(pred.distmod, na.rm = T), pred.n = length(pred.distmod)) %>% mutate(d.ci = qnorm(0.975)*d.sd/sqrt(d.n), pred.ci = qnorm(0.975)*pred.sd/sqrt(pred.n)) %>% mutate(data = "flock", time = "early")

#'get model preds fams late
fes.late = fams.expand.sub %>% filter(t_since_in > 60) %>% mutate(pred.distmod = predict(distmod_late, newdata = ., type = "response", allow.new.levels = T,re.form = ~(1|Breeding_year)+(1|Observer)+(1|Food_type))) %>% group_by(famsize) %>% summarise(d.mean = mean(distkol, na.rm = T), d.sd = sd(distkol, na.rm=T), d.n = length(distkol), pred.mean = mean(pred.distmod, na.rm = T), pred.sd = sd(pred.distmod, na.rm = T), pred.n = length(pred.distmod)) %>% mutate(d.ci = qnorm(0.975)*d.sd/sqrt(d.n), pred.ci = qnorm(0.975)*pred.sd/sqrt(pred.n)) %>% mutate(data = "flock", time = "late")
...
#pdf(file = "~/git/thesis/texts/fig2.pdf", height = 4, width = 4, title = "distance_famsize")
#
png(filename= "~/git/thesis/texts/fig2.png", height=1600, width = 1600, res = 300)
...
geom_pointrange(aes(x = famsize, y = d.mean, ymin = d.mean-d.ci, ymax = d.mean+d.ci, col = time, shape = time), fatten = 7, fill = "white", stroke = 1)+
...
#pdf(file = "~/git/thesis/texts/fig03.pdf", height = 4, width = 4, title = "famsize_time")

png(filename="~/git/thesis/texts/fig03.png", height = 1600, width=1600, res = 300)

#'point data
  
  ggplot(data = data.fig3)+
  geom_pointrange(aes(x = t_since_in, y = m.famsize, ymin = m.famsize - ci.famsize, ymax = m.famsize +ci.famsize, col = data, shape = data), fill = "white", fatten = 7, lty = 2, lwd = 0.5)+
  
  geom_ribbon(aes(x = t_since_in, ymin = m.pred-ci.pred, ymax = m.pred+ci.pred, group = data), fill = "grey", alpha = 0.5)+
  geom_line(aes(x = t_since_in, y = m.pred, col = data))+
  
  scale_shape_manual(values=c(21,24,22))+
  scale_colour_manual(values = c(2,4,1))+
  
  labs(list(x = "Days since arrival", y = "Mean number of juveniles"))+
  theme_bw()+a
...
ggplot(data = splits %>% filter(fam %in% c("Ev","Wo","Ha","Na","Ti","Ma","Ro","An","Jo")))+
  ...
  geom_point(data = splits %>% filter(fam %in% c("Ev","Wo","Ha","Na","Ti","Ma","Ro","An","Jo"), event == "end"), aes(x = t_since_in, y = juvs, group = fam), col = 1, pch = 4, size = 2)+
  geom_point(data = splits %>% filter(fam %in% c("Ev","Wo","Ha","Na","Ti","Ma","Ro","An","Jo"), event == "split"), aes(x = t_since_in, y = juvs+0.2, group = fam), shape = 21, fill = "white", col = 2, size = 5, stroke = 0.8)+
   geom_text(data = splits %>% filter(fam %in% c("Ev","Wo","Ha","Na","Ti","Ma","Ro","An","Jo"), event == "split"), aes(x = t_since_in, y = juvs+0.2, group = fam, label = juvs))+
  facet_wrap(~fam, nrow = 5, strip.position="right")+
  theme_minimal()+a+theme(strip.background=element_rect(fill="white", size = 0.7),
                          axis.line.x=element_line(), axis.line.y=element_line(), axis.ticks=element_line())+
  scale_y_continuous(breaks= c(0,5))+
    labs(list(x = "Days since arrival", y = "Number of juveniles"))
...
data.figA2 %>% 
ggplot()+
  geom_pointrange(aes(x = pin, y = m.famsize, ymin = m.famsize-ci.famsize, ymax = m.famsize+ci.famsize, group = data, col = data, shape = data), fill = "white", fatten= 7 , lty = 2, lwd = 0.5)+
...
#'get model pred
geese.fig5 = geese %>% mutate(flockpred = predict(flockmod, newdata = .,type = "response", allow.new.levels = T, re.form = NA))%>% group_by(distkol = round_any(distkol, 25)) %>% summarise(mean.flock = mean(flocksize, na.rm = T), mean.flockpred = mean(flockpred, na.rm = T), sd.flock = sd(flocksize, na.rm=T), sd.flockpred = sd(flocksize, na.rm = T), n.flock = length(flocksize), n.flockpred = length(flockpred)) %>% mutate(flock.ci = qnorm(0.975)*sd.flock/sqrt(n.flock), flockpred.ci = qnorm(0.975)*sd.flockpred/sqrt(n.flockpred))
```

```{r flock_distkol}
pdf(file = "~/git/thesis/texts/fig5b.pdf", width = 3, height = 3, title = "flock_dist")

geese.fig5 %>% filter(distkol <= 3000) %>% 
ggplot()+
  geom_pointrange(aes(x = distkol, y = mean.flock, ymin = mean.flock-flock.ci, ymax = mean.flock+flock.ci), fatten = 3, pch = 21, col = "black", fill = "white")+
  geom_line(aes(x = distkol, y = mean.flockpred), col = 1, lty = 2)+
  labs(list(x = "Distance from breeding area (km)", y = "Flock size"))+
  theme_bw()+a+
  ylim(NA,2000)
...
pred.fig5 = visreg(famsmod, xvar = "distkol", type = "conditional", scale = "response", plot=F)$fit
```
...
ggplot()+
  ...
  geom_pointrange(aes(x = distkol, y = mean.fams, ymin = mean.fams-fams.ci, ymax = mean.fams+fams.ci), fatten = 7, pch = 21, col = "black", fill = "white")+
geom_ribbon(data = pred.fig5, aes(x = distkol, ymin = visregLwr, ymax = visregUpr), fill = "grey", alpha = 0.5)+
  ...
  labs(list(x = "Distance from breeding area (km)", y = "Number of families"))+
  theme_bw()+a
...
#'get data
fams.fig4a = fams %>% 
  mutate(pfams = predict(famsmod, newdata=.,scale = "response", allow.new.levels = T)) %>% 
...
pred.fig4a = visreg(famsmod, "t_since_in", scale = "response", plot = F)$fit
```
...
ggplot()+
 ...
 # geom_line(aes(x = time, y = mean.pfams), col = 1, lty = 2)+
  labs(list(x = "Time since arrival (days)", y = "Successful families (n)"))+
  theme_bw()+a
...
pred.fig4b = visreg(famsmod, "flocksize", scale="response", plot = F)$fit
```
...
fams.fig4b %>% 
  ...
  labs(list(y = "Successful families (n)", x = "Flock size"))+ylim(0, 130)
dev.off()
```
...
flock.fig4c %>% 
  ...
  theme_bw()+a+
  labs(list(x = "Time since arrival (days)", y = "Juvenile proportion"))+ylim(0.1, 0.4)+xlim(NA,180)
...
theme_bw()+a+
labs(list(y = "Juvenile proportion", x = "Flock size"))+
xlim(NA, 1e3)+ylim(0.05,0.35)
