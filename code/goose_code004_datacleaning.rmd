...
fams.coords = fams.coords[fams.coords$lat > 50 & fams.coords$lat <56 &
...
missing.sites = goose.clean %>% filter(is.na(lon)) %>% summarise(sites = unique(Site_name))

#'geocode for these sites
#missing.coords = geocode(missing.sites$sites)

#'bind missing sites and coords
#missing.sites = cbind(missing.sites, missing.coords)
```
