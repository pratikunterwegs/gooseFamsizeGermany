...
fams2016 = fams2016[fams2016$location_lat < 56 & fams2016$location_long < 7,]
```
...
fams2016.4 = lapply(fams2016.3, function(x){lapply(x, function(y){y = data.frame(lon = y$location_long, lat = y$location_lat, v = y$ground_speed, t = y$timestamp, fam = y$fam, lag = c(0,timeLag(y)))})})

```

```{r filter_by_lag}
#'remove lags below 1000, where speed is also above 2
fams2016.4 = lapply(fams2016.4, function(x){lapply(x, function(y){y = y %>% filter(lag >= 600, lag <= 2000)})})
...
#'filter
fams2014.3 = fams2014.3 %>% filter(lon <=7, lat <=56,            error < quantile(error, seq(0,1,0.1))[10])
...
```{r exportfams2014}
save(fams2014.5, file = "fams2014.nobursts.rdata")
```
