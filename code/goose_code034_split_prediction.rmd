...
#'count flights
famstats =  data.frame(famcoords %>% group_by(fam, date = round_date(time, "day")) %>% summarise(mdist = mean(dist, na.rm = T), ddist = sum(na.omit(dist)), days = mean(days, na.rm = T), fsize = min(fsize), flights = sum(flight), split = sum(split), mlon = mean(lon, na.rm = T), mlat = mean(lat, na.rm = T)))
