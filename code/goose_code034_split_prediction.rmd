...
f6a =
  ...
  geom_smooth(data = famstats, aes(x = juvs, y = p), col = "grey20", lwd = 1, method = "loess")+
  labs(list(x = "Family size", y= "Daily split probability", title = NULL))+
  theme.pub()+
  coord_cartesian()
...
f6b =
  ...
  geom_smooth(data = famstats, aes(x = ddist/1e3, y = p), col = "grey20", lwd = 1)+
  labs(list(x = "Distance travelled/day (km)", y= "Daily split probability", title = NULL))+
  theme.pub()+
...
```{r}
cairo_ps(filename = "figs/fig5.eps", height = 4, width = 6)
