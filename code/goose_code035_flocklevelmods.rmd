...
famprop.mod = glmer(famprop ~ sqrt(flocksize) + sqrt(distkol) + (t_since_in)+ (1|Breeding_year) + (1|Observer), data = fambirds %>% filter(famprop <= 1, flocksize <= 5000), family = "binomial")
```

```{r famjuvsprop}
#'what proportion of a flock's juveniles are associated in families, rather than being independent?

famjuvprop.mod = glmer(famjuvsprop ~ sqrt(flocksize) + sqrt(distkol) + (t_since_in) + (1|Breeding_year) + (1|Observer), data = fambirds %>% filter(famprop <= 1, flocksize <=5000), family = "binomial")
```
...
juvprop.mod = glmer(propjuv ~ sqrt(flocksize) + sqrt(distkol) + (t_since_in) + (1|Breeding_year) + (1|Observer), data = geese %>% filter(flocksize>0), family = "binomial")
