...
fambirds = fams.expand %>% group_by(flockid) %>% summarise(fambirds = length(famsize)*2 + sum(famsize), flocksize = first(flocksize), famprop = fambirds/flocksize, t_since_in = first(t_since_in), distkol = first(distkol), mintemp = first(mintemp), ppt = first(ppt), windspeed = first(windspeed), Breeding_year = first(Breeding_year), Observer = first(Observer), Food_type = first(Food_type), famjuvsprop = sum(famsize)/first(juvs), zone = first(zone)) %>% filter(famprop <= 1, famjuvsprop <=1)
```
...
famprop.mod = glmer(famprop ~ sqrt(flocksize) + sqrt(distkol) + (t_since_in)+ (1|Breeding_year) + (1|Observer) + (1|zone), data = fambirds %>% filter(famprop <= 1, flocksize <= 5000), family = "binomial")
```

```{r famjuvsprop}
#'what proportion of a flock's juveniles are associated in families, rather than being independent?

famjuvprop.mod = glmer(famjuvsprop ~ sqrt(flocksize) + sqrt(distkol) + (t_since_in) + (1|Breeding_year) + (1|Observer) + (1|zone), data = fambirds %>% filter(famprop <= 1, flocksize <=5000), family = "binomial")
```
...
juvprop.mod = glmer(propjuv ~ sqrt(flocksize) + sqrt(distkol) + (t_since_in) + (1|Breeding_year) + (1|Observer) + (1|zone), data = geese %>% filter(flocksize>0, flocksize <= 5000), family = "binomial")
...
flocksize.mod = glmer(flocksize ~ sqrt(distkol) + log(t_since_in) + mintemp + (1|Observer) + (1|zone)+ (1|Breeding_year), data = geese, family = "poisson")
```
...
```{r juvsclim}
save(famprop.mod, famjuvprop.mod, juvprop.mod, flocksize.mod, fambirds, geese, geeseorg,file = "mods_flocklevel.rdata")
```
