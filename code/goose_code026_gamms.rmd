...
#'remove t_to_out
mod03 = glmer(famsize ~ lon + flocksize + t_since_in + p.index + (1|Breeding_year) + (1|Observer) + (1|Food_type), data = fams.expand.sub, family = "poisson")
```
...
```{r gorg.m03}
mod03.gorg = glmer(famsize ~ lon + t_since_in + p.index + (1|neckring) + + (1|breedyr/neckring), data = geeseorg, family = poisson)
```
