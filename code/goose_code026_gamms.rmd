...
```{r}
distmod = gamm4(distkol ~ s(famsize) + (t_since_in), random = ~ (1|Breeding_year) + (1|Food_type) + (1|Observer), data = fams.expand.sub, family = "gaussian")

distmod2 = glmer(distkol ~ (famsize) + (t_since_in) + (1|breedyr) + (1|neckring), data = geeseorg, family = "gaussian")
```
...
```{r gorg.m03}
mod03.gorg = glmer(famsize ~ distkol + t_since_in + p.index + (1|neckring) + + (1|breedyr/neckring), data = geeseorg, family = poisson)
```
...
```{r gorg.success}
mod03.gorg.s = glmer(famsize ~ lon + t_since_in + p.index + (1|neckring) + + (1|breedyr/neckring), data = geeseorg %>% filter(famsize > 0), family = poisson)
```
