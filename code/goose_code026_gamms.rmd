...
```{r m03}
#'remove t_to_out
mod03 = bam(famsize ~ lon + flocksize + t_since_in  + s(Food_type, bs = "re") + s(Observer, bs = "re") + s(Breeding_year, bs = "re") + s(Site_name, bs = "re"), data = fams.expand.sub, family = "poisson")
```
...
```{r gorg.m03}
mod03.gorg = bam(famsize ~ lon + t_since_in + s(id, bs = "re") + s(breedyr, bs = "re"), data = geeseorg, family = poisson)
```
