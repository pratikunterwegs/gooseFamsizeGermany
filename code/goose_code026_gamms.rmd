...
```{r}
distmod = lmer(distkol ~ famsize + sqrt(t_since_in) + (1|Breeding_year) + (1|Observer) + (1|zone), data = fams.expand %>%  filter(t_since_in < 60))

distmod_late = lmer(distkol ~ (famsize) + sqrt(t_since_in) + (1|Breeding_year) + + (1|zone) + (1|Observer), data = fams.expand %>% filter(t_since_in > 60))

distmod2 = glmer(distkol ~ (famsize) + sqrt(t_since_in) + (1|breedyr) + (1|neckring), data = geeseorg %>% filter(t_since_in < 60), family = "gaussian")

distmod2.1 = glmer(distkol ~ (famsize) + sqrt(t_since_in) + (1|breedyr) + (1|neckring), data = geeseorg %>% filter(t_since_in < 60, famsize >0), family = "gaussian")

distmod2_late = glmer(distkol ~ (famsize) + sqrt(t_since_in) + (1|breedyr) + (1|neckring), data = geeseorg %>% filter(t_since_in > 60), family = "gaussian")
