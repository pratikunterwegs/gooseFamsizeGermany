...
distmod2 = glmer(distkol ~ (famsize) + t_since_in + (1|breedyr/neckring) + (1|neckring), data = geeseorg %>% filter(t_since_in < 60), family = "gaussian")

distmod2_late = glmer(distkol ~ (famsize) + t_since_in + (1|breedyr/neckring) + (1|neckring), data = geeseorg %>% filter(t_since_in > 60), family = "gaussian")
```
