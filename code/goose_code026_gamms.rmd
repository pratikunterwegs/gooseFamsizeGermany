...
distmod2 = glmer(distkol ~ (famsize) + t_since_in + (1|breedyr) + (1|neckring), data = geeseorg %>% filter(t_since_in < 60), family = "poisson")

distmod2.1 = glmer(distkol ~ (famsize) + t_since_in + (1|breedyr) + (1|neckring), data = geeseorg %>% filter(t_since_in < 60, famsize >0), family = "poisson")

distmod2_late = glmer(distkol ~ (famsize) + t_since_in + (1|breedyr) + (1|neckring), data = geeseorg %>% filter(t_since_in > 60), family = "poisson")

distmod2.1_late = glmer(distkol ~ (famsize) + t_since_in + (1|breedyr) + (1|neckring), data = geeseorg %>% filter(t_since_in > 60, famsize >0), family = "poisson")
```
...
# Family size cold models

```{r cold_mods}
library(mgcv)

coldmod1 = bam(famsize ~ distkol + s(mintemp) + s(windspeed) + s(ppt) + ti(mintemp,windspeed), data = fams.expand.sub, family = "poisson")
```
...
visreg(coldmod1, "mintemp", scale = "response")
```

# Proportion of successful birds by p.index

```{r}
g1 = geeseorg %>% group_by(breedyr) %>% summarise(p.index = mean(p.index, na.rm = T), s = sum(success))
g2 = geeseorg %>% group_by(breedyr) %>% count()
