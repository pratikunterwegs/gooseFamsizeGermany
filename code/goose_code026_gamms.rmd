...
mod03full = bam(famsize ~ lon + flocksize + t_since_in  + p.index + s(Food_type, bs = "re") + s(Observer, bs = "re") + s(Breeding_year, bs = "re") + s(Site_name, bs = "re"), data = fams.expand, family = "poisson", na.action = na.omit)
```
...
```{r gorg.success}
mod03.gorg.s = bam(famsize ~ lon + t_since_in + p.index + s(id, bs = "re") + s(breedyr, bs = "re"), data = geeseorg %>% filter(famsize > 0), family = poisson)
```
